...
def get_active_verification(shelter_id: uuid.UUID) -> Optional[ShelterVerification]:
    ...
    return ShelterVerification.objects.filter(
...
def get_verification_by_id(
    ...
    """Fetches a verification workflow instance by ID."""
    return ShelterVerification.objects.filter(id=verification_id).first()
